-- Ensure ALL players in each completed match have ratings from ALL other players
-- This fills in any missing rating pairs
...
BEGIN
  ...
  LOOP
    -- For each player who will rate others
    FOR rater_rec IN 
    ...
    LOOP
      -- Rate every other player in the match
      FOR rated_rec IN 
        SELECT mp.user_id FROM match_players mp 
        WHERE mp.match_id = match_rec.match_id 
          AND mp.user_id IS NOT NULL 
          AND mp.user_id != rater_rec.user_id
          AND mp.join_status = 'confirmed'
      LOOP
        ...
        SELECT EXISTS(
          ...
          WHERE match_id = match_rec.match_id 
            AND rater_user_id = rater_rec.user_id 
            AND rated_user_id = rated_rec.user_id
        ) INTO rating_exists;
        
        -- Only insert if doesn't exist
        IF NOT rating_exists THEN
          -- Determine base rating based on performer tier
          IF rated_rec.user_id = ANY(top_performers) THEN
            base_rating := 4 + floor(random() * 2)::int; -- 4-5
          ELSIF rated_rec.user_id = ANY(mid_performers) THEN
